...
from videollava.constants import IMAGE_TOKEN_INDEX, DEFAULT_IMAGE_TOKEN, DEFAULT_IM_START_TOKEN, DEFAULT_IM_END_TOKEN, \
    DEFAULT_VIDEO_TOKEN, DEFAULT_AUDIO_TOKEN
from videollava.conversation import conv_templates, SeparatorStyle
from videollava.model.builder import load_pretrained_model
from videollava.serve.utils import load_image, image_ext, video_ext, audio_ext
...
def main(args):
    ...
    image_processor, video_processor, audio_processor = processor['image'], processor['video'], processor['audio']
    ...
    for file in args.file:
        if os.path.splitext(file)[-1].lower() in image_ext:
            ...
            file = audio_processor(file, return_tensors='pt')['pixel_values'][0].to(model.device, dtype=torch.float16)
            special_token += [DEFAULT_IMAGE_TOKEN]
        else:
            raise ValueError(f'Support video of {video_ext} and image of {image_ext} and audio of {audio_ext}, but found {os.path.splitext(file)[-1].lower()}')
        print(file.shape)
        tensor.append(file)
    ...
    while True:
        ...
        # outputs_zh = asyncio.run(translator.translate(outputs, src='en', dest='zh-cn')).text
        # print("雅琪:", outputs_zh.split("</s>")[0])
        # tts = gTTS(text=outputs_zh.split("</s>")[0], lang='zh', slow=False)
        # tts.save("output.mp3")
