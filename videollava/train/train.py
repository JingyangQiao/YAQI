...
from videollava.constants import IGNORE_INDEX, IMAGE_TOKEN_INDEX, DEFAULT_IMAGE_TOKEN, DEFAULT_IM_START_TOKEN, \
    DEFAULT_IM_END_TOKEN, DEFAULT_VIDEO_TOKEN, DEFAULT_VID_START_TOKEN, DEFAULT_VID_END_TOKEN, DEFAULT_AUDIO_TOKEN, \
    DEFAULT_AUD_START_TOKEN, DEFAULT_AUD_END_TOKEN, MAX_IMAGE_LENGTH, MAX_VIDEO_LENGTH, MAX_AUDIO_LENGTH
...
class ModelArguments:
    ...
    mm_tunable_parts: Optional[str] = field(
        default=None, metadata={"help": 'Could be "mm_mlp_adapter", "mm_mlp_adapter,mm_language_model"'}
    )
...
def safe_save_model_for_hf_trainer(trainer: transformers.Trainer, output_dir: str):
    ...
    if getattr(trainer.args, "tune_mm_mlp_adapter", False) or (hasattr(trainer.args, "mm_tunable_parts") and (len(trainer.args.mm_tunable_parts.split(",")) == 1 and ("mm_mlp_adapter" in trainer.args.mm_tunable_parts))):
        # Only save Adapter
...
def preprocess_multimodal(
    ...
    for source in sources:
        for sentence in source:
            ...
            # ======================================================================================================
            if sentence['value'].startswith(DEFAULT_IMAGE_TOKEN) or sentence['value'].endswith(DEFAULT_IMAGE_TOKEN) or \
            sentence['value'].startswith(DEFAULT_VIDEO_TOKEN) or sentence['value'].endswith(DEFAULT_VIDEO_TOKEN) or \
            sentence['value'].startswith(DEFAULT_AUDIO_TOKEN) or sentence['value'].endswith(DEFAULT_AUDIO_TOKEN):  # run with multi-im, multi-vid, multi-im & multi-vid
            ...
            replace_token, vid_replace_token, aud_replace_token = DEFAULT_IMAGE_TOKEN, DEFAULT_IMAGE_TOKEN * data_args.num_frames, DEFAULT_IMAGE_TOKEN
            if data_args.mm_use_im_start_end:
                replace_token = DEFAULT_IM_START_TOKEN + replace_token + DEFAULT_IM_END_TOKEN
                vid_replace_token = DEFAULT_VID_START_TOKEN + vid_replace_token + DEFAULT_VID_END_TOKEN
                aud_replace_token = DEFAULT_AUD_START_TOKEN + aud_replace_token + DEFAULT_AUD_END_TOKEN
            ...
            sentence['value'] = sentence['value'].replace(DEFAULT_AUDIO_TOKEN, aud_replace_token)
...
class LazySupervisedDataset(Dataset):
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        if True:
            ...
            # ======================================================================================================
            if 'image' in sources[0] and 'video' not in sources[0] and 'audio' not in sources[0]:
                # rank0_print('image')
            ...
            elif 'image' not in sources[0] and 'video' in sources[0] and 'audio' not in sources[0]:
                # rank0_print('video')
            ...
            elif 'image' not in sources[0] and 'video' not in sources[0] and 'audio' in sources[0]:
            ...
            elif 'image' in sources[0] and 'video' not in sources[0] and 'audio' in sources[0]:
                # audio->image
                audio_file = self.list_data_dict[i]['audio']
                audio_folder = self.data_args.audio_folder
                audio_processor = self.data_args.audio_processor
            ...
            elif 'image' in sources[0] and 'video' in sources[0] and 'audio' not in sources[0]:
            ...
            elif 'image' in sources[0] and 'video' in sources[0] and 'audio' in sources[0]:
                # audio->video->image
                audio_file = self.list_data_dict[i]['audio']
                audio_folder = self.data_args.audio_folder
                audio_processor = self.data_args.audio_processor
                ...
                image = audio + video + image  # audio->video->image
            ...
            if 'image' in self.list_data_dict[i] or 'video' in self.list_data_dict[i] or 'audio' in self.list_data_dict[i]:
                data_dict['image'] = image
                # print("泸州月光", data_dict['image'])
...
def train():
    ...
    # ==========================================================================
    if model_args.image_tower is not None or model_args.video_tower is not None or model_args.audio_tower is not None:
    # ==========================================================================
    ...
    # =============================================================================================================
        ...
        if model_args.mm_tunable_parts is None:  # traditional way of deciding which part to train
            ...
            model.config.unfreeze_mm_image_tower = model_args.unfreeze_mm_image_tower
            if model_args.unfreeze_mm_image_tower:
                image_tower.requires_grad_(True)
            else:
                image_tower.requires_grad_(False)

            model.config.unfreeze_mm_video_tower = model_args.unfreeze_mm_video_tower            
            if model_args.unfreeze_mm_video_tower:
                video_tower.requires_grad_(True)
            else:
                video_tower.requires_grad_(False)

            model.config.unfreeze_mm_audio_tower = model_args.unfreeze_mm_audio_tower
            if model_args.unfreeze_mm_audio_tower:
                audio_tower.requires_grad_(True)
            else:
            ...
            model.requires_grad_(False)
            if model_args.image_tower:
                image_tower.requires_grad_(False)
            if model_args.video_tower:
                video_tower.requires_grad_(False)
            ...
            if "mm_language_model" in tunable_parts:
                for name, param in model.named_parameters():
                    if "image_tower" not in name and "video_tower" not in name and "audio_tower" not in name and "mm_projector" not in name:
