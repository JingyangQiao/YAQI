...
class LlavaMetaModel:
    ...
    def __init__(self, config):
        ...
        if getattr(config, "mm_image_tower", None) is not None or getattr(config, "mm_video_tower", None) or getattr(config, "mm_audio_tower", None) is not None:
...
class LlavaMetaForCausalLM(ABC):
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        if (image_tower is None and video_tower is None and audio_tower is None) or images is None or input_ids.shape[1] == 1:
            if past_key_values is not None and (image_tower is not None or video_tower is not None or audio_tower is not None) and images is not None and input_ids.shape[1] == 1:
        ...
        image_idx = [idx for idx, img in enumerate(images) if img.ndim == 3 and img.size(-1) == 224]
        video_idx = [idx for idx, vid in enumerate(images) if vid.ndim == 4]
        audio_idx = [idx for idx, aud in enumerate(images) if aud.ndim == 3 and aud.size(-1) != 224]
        ...
        audios_minibatch = torch.stack([images[idx] for idx in audio_idx]) if len(audio_idx) > 0 else []  # mini_b h w
        ...
        if getattr(audios_minibatch, 'ndim', 0) == 4:  # batch consists of audios, [mini_b, h, w]
            audio_features_minibatch = self.encode_audios(audios_minibatch)  # fake list [mini_b, c]
